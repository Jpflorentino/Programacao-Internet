...
module.exports = (elasticSearch) => {
    ...
    function createGroup(name, description) {
        const url = baseUrl + '/_doc' + '?refresh=wait_for'
    ...
    function editGroup(groupId, name, description) {
        ...
        const url = baseUrl + '/_doc' + `/${groupId}` + '?refresh=wait_for'
    ...
    function addGameToGroup(game, groupId) {
        ...
        return getSpecificGroup(groupId).then(group => {
            if (group.games.some(element => element.name === game[0].name)) {
                return Promise.reject({
                    code: 400,
    ...
    function gamesBetweenMinMax(groupId, minRating, maxRating) {
        ...
        return getSpecificGroup(groupId).then(group => {
            const groups = group.games.filter(game => game.total_rating >= minRating && game.total_rating <= maxRating)
            if (groups.length === 0) {
                return Promise.reject({
                    code: 404,
